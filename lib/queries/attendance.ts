...
/**
 * Students whose attendance has fallen below a threshold — the list a
 * principal actually acts on, rather than a school-wide average that hides
 * the individuals in trouble.
 */
export async function getAttendanceConcerns(
  ...
  const groups = await prisma.attendanceRecord.groupBy({
    by: ["studentId", "status"],
    where: sectionIds === "ALL" ? {} : { sectionId: { in: sectionIds } },
    _count: { _all: true },
  });

  const byStudent = new Map<string, { present: number; total: number }>();
  for (const group of groups) {
    const entry = byStudent.get(group.studentId) ?? { present: 0, total: 0 };
    entry.total += group._count._all;
    if (group.status === "PRESENT" || group.status === "LATE") entry.present += group._count._all;
    byStudent.set(group.studentId, entry);
  }

  const below = [...byStudent.entries()]
    // A handful of marked days can't establish a pattern; requiring a
    // minimum sample stops a student who joined last week from topping the
    // list on a single absence.
    .filter(([, { total }]) => total >= 10)
    .map(([studentId, { present, total }]) => ({ studentId, percent: (present / total) * 100, total }))
    .filter((entry) => entry.percent < thresholdPercent)
